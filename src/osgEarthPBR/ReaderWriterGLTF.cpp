...
public:
    ...
    ReadResult readNode(const std::string& location, const osgDB::Options* options) const
    {
        ...
        if (ext == "pbr")
        {
            filepath = osgDB::getNameLessExtension(location);
            ext = osgDB::getFileExtension(filepath);
        }
        ...
        if (ext == "gltf")
        {
            ...
            return reader.read(filepath, false, options);
        }
        else if (ext == "glb")
        {
            GLTFReader reader;
            reader.setTextureCache(&_cache);
            tinygltf::Model model;
            return reader.read(filepath, true, options);
        }
        else if (ext == "b3dm")
        {
            std::string data = URI(filepath).getString(options);
            B3DMReader reader;
            reader.setTextureCache(&_cache);
