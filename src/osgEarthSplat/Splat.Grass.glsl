...
void oe_Grass_VS(inout vec4 vertex)
{
	...
	//boost alpha at distance to make vegetation more dens 
	//const float alpha_boost_fade_dist = 50;
	//float alpha_boost = 1.0 + 1.0 * clamp(-zv / alpha_boost_fade_dist, 0.0, 1.0);
	//falloff *= alpha_boost;
...
void oe_Grass_FS(inout vec4 color)
{
...
#ifdef OE_GROUNDCOVER_COLOR_SAMPLER
    float modulation = 0.9;//oe_grass_color_modulation;
	...
	if(grenness < 0.05)
	//float grenness = 2.0 * norm_color.g - 2 * norm_color.r + norm_color.b;
	//if(grenness < 0.4)
	{
		discard;
	}
